...
// El primer chequeo lo dispara window.onload (diferido 5s).
// A partir de ahí, cada 30 segundos.
setTimeout(()=> setInterval(chequearRecordatorios, 30000), 5000);
...
window.onload = function(){
  // Mostrar saludo de texto inmediatamente (nunca bloquea)
  const div = document.createElement("div");
  ...
  // Audio de saludo: solo se intenta si el usuario ya interactuó con la página.
  // Si el navegador lo bloquea (autoplay policy), se ignora silenciosamente.
  const intentarSaludo = ()=>{
    ...
    saludoAudio.play().catch(()=>{
      // Autoplay bloqueado por el navegador — no hacer nada, la página ya cargó bien
    });
    // Quitar listener después del primer intento
    document.removeEventListener("click", intentarSaludo);
    document.removeEventListener("keydown", intentarSaludo);
  };

  // Intentar reproducir al primer gesto del usuario
  document.addEventListener("click",   intentarSaludo, { once: true });
  document.addEventListener("keydown", intentarSaludo, { once: true });

  // Wake word: diferido 2 segundos para no bloquear la carga inicial
  setTimeout(()=>{
    try { iniciarActivador(); } catch(e){ console.warn("Wake word no disponible:", e); }
  }, 2000);

  // Chequeo de recordatorios: diferido para no disparar fetch en el mismo tick de carga
  setTimeout(chequearRecordatorios, 5000);
};
</script>
