...
#header button, #header select{
  font-size:14px; padding:6px 10px; border-radius:4px; border:1px solid #999; cursor:pointer;
  background:#001f2e; color:#00eaff; box-shadow:none; transition:0.3s;
}
...
.message{
 ...
 opacity:0;
 transition:opacity 0.5s, color 0.3s, background 0.3s;
 font-size:15px;
}
...
#adjuntos_menu button, #premiumMenu button{
  background:#fff;
  border:none;
  color:#000;
  width:100%;
...
@media (max-width:600px){
  #inputBar input[type=text]{ font-size:16px; padding:8px; }
  #inputBar button,#header button,#header select{ font-size:14px; padding:6px; }
  #logo{ width:140px; }
}
</style>
</head>
...
<div id="header">
  <div id="leftButtons">
    ...
    <div id="premiumContainer" style="position:relative; margin-left:12px;">
      ...
      <div id="premiumMenu" style="display:none;position:absolute;top:36px;left:0;">
...
function agregar(msg,cls,imagenes=[]){ let c=document.getElementById("chat"),div=document.createElement("div"); div.className="message "+cls; div.innerHTML=msg; c.appendChild(div); setTimeout(()=>div.classList.add("show"),50); imagenes.forEach(url=>{ let img=document.createElement("img"); img.src=url; div.appendChild(img); }); c.scroll({top:c.scrollHeight,behavior:"smooth"}); return div; }
...
function cambiarModo(modo){
  ...
  switch(modo){
    case "neon":
      ...
      botones.forEach(b=>{ b.style.background="#001f2e"; b.style.color="#00eaff"; b.style.border="1px solid #006688"; });
      mensajes.forEach(m=>{ m.style.color = m.classList.contains("user") ? "#b4b7ff" : "#00eaff"; });
      break;
    case "black":
      ...
      botones.forEach(b=>{ b.style.background="#222"; b.style.color="#fff"; b.style.border="1px solid #555"; });
      mensajes.forEach(m=>{ m.style.color = "#fff"; });
      break;
    case "white":
      body.style.background="#fff"; body.style.color="#000";
      chat.style.background="#fff"; inputBar.style.background="#fff"; header.style.background="#fff";
      botones.forEach(b=>{ b.style.background="#fff"; b.style.color="#000"; b.style.border="1px solid #ccc"; });
      mensajes.forEach(m=>{ m.style.color = "#000"; });
      break;
  }
}
